...
public class ad_result extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if ("buy".equals(action)) {
            ...
            borrowAdExists(searchedBookISBN);
        }
    }
    ...
    private void borrowAdExists(String isbn) {
        // Find the main LinearLayout container
        LinearLayout mainLayout = findViewById(R.id.mainLayout);

        // Retrieve borrow ads with borrower's name
        List<BorrowAd> borrowAds = dbHandler.getBorrowAdByIsbn(isbn);

        if (borrowAds.isEmpty()) {
            showNoAdsMessage(mainLayout);
        } else {
            getBorrowAds(mainLayout, borrowAds);
        }
    }

    private void getBorrowAds(LinearLayout mainLayout, List<BorrowAd> borrowAds) {
        // Create a container layout for each ad and its button
        for (BorrowAd ad : borrowAds) {
            ...
            // Create a text view for the ad
            TextView adTextView = new TextView(this);
            adTextView.setText("Οναματεπώνυμο Κατόχου: " + dbHandler.getUserNameById(ad.getBorrowPublisher()) + "\n" + "Κατάσταση: " + ad.getBorrowStatus() + "\n" + "Τοποθεσία: " + ad.getBorrowLocation());
            adContainerLayout.addView(adTextView);
            ...
            adButton.setOnClickListener(v -> showBorrowAdDetails(ad.getBorrowAdId()));
            adContainerLayout.addView(adButton);
    ...
    private void showBorrowAdDetails(int adId) {
        // Pass the ad ID to the ad_details activity
